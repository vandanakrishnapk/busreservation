...
<div class="row no-gutters">
          <form action="{{ route('search') }}" method="get">
            <select name="location" class="form-control select">
              <option value="">Going_to</option>
              @foreach($item as $items)
              <option value="{{ $items->id }}"> {{ $items->places }} </option>
              @endforeach
            </select>
   
            <input type="datetime-local" name="date" class="form-control date mt-1">

            <select name="No_of_seats" class="form-control select mt-2">
              <option value="">capacity</option>
              @foreach($capacity as $capacities)
              <option value="{{ $capacities->id }}">{{ $capacities->No_of_seats }} </option>
              @endforeach
            </select>
            <select name="bus_type" class="form-control select mt-2">
              <option value="">AC Non-AC</option>
              @foreach($bustype as $bustypes)
              <option value="{{ $bustypes->id }}">{{ $bustypes->bus_types }} </option>
              @endforeach
            </select>
